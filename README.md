...
- `RULE_CORPUS`/`OBLIGATION_MAP`/`OBLIGATION_RULE_CHUNKS` are now populated (2026-09-14) — five real citations from the FSA's own FIEA translation and Osaka Exchange's own Operational Procedures document, one per detector family, approved via a real `INFORMATION_SCHEMA`-validated gate (`SP_APPROVE_OBLIGATION`). `REPORT_TEMPLATE_RULE_CHUNKS` (field-level report-format citations) is still empty — see NOTES.md.
...
- `narrative_draft.py` has no CLI execution wrapper yet (`assure_report.py`/`rule_interpret.py` both gained one; `narrative_draft.py` still takes pre-fetched Python objects as arguments).
