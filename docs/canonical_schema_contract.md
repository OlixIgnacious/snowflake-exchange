...
**v7 closes an inconsistency v6 introduced but never resolved: what happens when a circular requires a field the schema has no source for at all.** v6 described `REPORT_TEMPLATES.SOURCE_MAPPING` as "validated by the same `GOVERNANCE_WRITE` approval-time `INFORMATION_SCHEMA` check" as `OBLIGATION_MAP` — but unlike `OBLIGATION_MAP`, `REPORT_TEMPLATES` was never actually given a `STATUS`/approval lifecycle to hang that check off, so there was no defined answer for the real case: a rule-interpret gap analysis finds a required field in a new circular, and nothing in `VIGIL.CORE` — no named column, no `REGULATORY_ATTRIBUTES` key — currently captures it. Left unaddressed, this forces a bad choice between blocking the entire template indefinitely, fabricating a mapping that doesn't really resolve, or silently leaving the field off the template so `FIELDS_COMPLETE` reports false confidence. Fixed in `[FIX #27]`–`[FIX #29]`: `REPORT_TEMPLATES` gets its own `STATUS` (`proposed`/`mapped`/`gap`) mirroring `OBLIGATION_MAP`'s, `gap` is a deliberate, non-blocking, tracked state for exactly this case, and a new `REPORT_TEMPLATE_COVERAGE` view surfaces it — the same "surfaced, not hidden" discipline `WASH_DETECTION_COVERAGE` (Fix #3) already applies to wash-trading blind spots, now applied a second time to reporting blind spots.
...
## [FIX #27] `REPORT_TEMPLATES` had no `STATUS`/approval lifecycle of its own

v6 said `SOURCE_MAPPING` is "validated by the same `GOVERNANCE_WRITE` approval-time `INFORMATION_SCHEMA` check" as `OBLIGATION_MAP` (Fix #9) — but that check has to attach to something, and `OBLIGATION_MAP` has a `STATUS` column (`proposed`/`approved`) for it to gate; `REPORT_TEMPLATES` as originally written had no such column at all. An internal inconsistency: language borrowed from a governance pattern without actually adopting the structure that pattern depends on. Fixed: `REPORT_TEMPLATES` gains `STATUS VARCHAR NOT NULL DEFAULT 'proposed'` (`proposed` / `mapped` / `gap` — see Fix #28 for why a third state, not just `OBLIGATION_MAP`'s two). A transition is a new row, same (`JURISDICTION_ID`, `REPORT_TYPE`, `FIELD_NAME`), later `LOADED_AT` — the identical append-only pattern every `STATUS` transition in this schema already follows, `GOVERNANCE_WRITE` remains `INSERT`-only (no new RBAC grant needed).

## [FIX #28] A required field with no current data source had nowhere to go

The real case this closes: `rule-interpret`'s gap analysis finds a field a circular requires, and nothing in `VIGIL.CORE` — no named column, no `REGULATORY_ATTRIBUTES` key an adaptor currently populates — captures it. Under a plain `proposed`/`mapped` `STATUS`, this forces a bad choice: block the field (and arguably the whole template) from ever reaching `mapped` indefinitely, fabricate a `SOURCE_MAPPING` that doesn't actually resolve just to get past the `INFORMATION_SCHEMA` check, or quietly omit the field from `REPORT_TEMPLATES` entirely so `FIELDS_COMPLETE` reports false confidence about a report the regulator would consider incomplete. All three are worse than admitting the gap.

Fixed: `STATUS = 'gap'` is a deliberate, valid, non-blocking state — `SOURCE_MAPPING` is `NULL` by convention when `STATUS = 'gap'` (not a resolution failure; a `mapped` row's `SOURCE_MAPPING` must still resolve against `INFORMATION_SCHEMA` per Fix #9's discipline). A `gap` field is not exempted from `IS_REQUIRED` — the regulator still requires it — it is exempted from blocking report generation while the underlying ingestion gap gets built. New companion view `REPORT_TEMPLATE_COVERAGE`, one row per (`JURISDICTION_ID`, `REPORT_TYPE`), `PCT_REQUIRED_FIELDS_MAPPED` (the fraction of `IS_REQUIRED` fields in `REPORT_TEMPLATES_CURRENT` with `STATUS = 'mapped'`) plus the list of `gap` field names — the same "surfaced, not hidden" discipline `WASH_DETECTION_COVERAGE` (Fix #3) already applies to wash-trading blind spots. `rule-interpret`/`assure-report` surface this figure alongside any completeness check, so "this report is complete" and "this report is complete among the fields we're currently able to capture" are never presented as the same claim. Closing a gap (an adaptor is updated, a new column ships) is itself just a new row — `gap` → `mapped`, same key, later `LOADED_AT` — never a mutation of the `gap` row.

## [FIX #29] `FIELDS_COMPLETE` (Fix #22) would have measured against `gap` fields it can structurally never satisfy

Left as originally defined, `FIELDS_COMPLETE` checked every `IS_REQUIRED` row in `REPORT_TEMPLATES_CURRENT` — including a `gap` row, which by definition has no source and can never be populated. Every report of that type would permanently show incomplete, which is true but useless as a per-report signal — it's a template-level finding (`REPORT_TEMPLATE_COVERAGE`, Fix #28), not evidence that *this particular report* was mishandled. Fixed: `FIELDS_COMPLETE` is computed against `REPORT_TEMPLATES_CURRENT WHERE IS_REQUIRED AND STATUS = 'mapped'` only — a `gap` field's absence is `REPORT_TEMPLATE_COVERAGE`'s finding to report, not `FIELDS_COMPLETE`'s, so the two checks stay distinct instead of one silently swallowing the other.
...
| `FIELDS_COMPLETE` | BOOLEAN | Computed against `REPORT_TEMPLATES_CURRENT WHERE IS_REQUIRED AND STATUS = 'mapped'` for this report's (`JURISDICTION_ID`, `REPORT_TYPE`) (Fix #22, #29) — not an unspecified black box, and not measured against a `gap` field it could never satisfy; see `REPORT_TEMPLATE_COVERAGE` (Fix #28) for that separate finding. |
...
### `REPORT_TEMPLATES` — new, per Fix #21; `STATUS` lifecycle added per Fix #27
| Column | Type | Notes |
|---|---|---|
| `JURISDICTION_ID` | VARCHAR NOT NULL | |
| `REPORT_TYPE` | VARCHAR NOT NULL | e.g. `transaction_report`, `large_trade_notification`, `position_report`. |
| `FIELD_NAME` | VARCHAR NOT NULL | The regulator's own field/tag name (e.g. `Buyer_LEI`, `Trading_Capacity`) — not a `VIGIL.CORE` column name. |
| `FIELD_ORDER` | NUMBER | Position in the format, when the format is order-sensitive (fixed-width/CSV); NULL for tag-based formats (XML/JSON) where order doesn't matter. |
| `STATUS` | VARCHAR NOT NULL DEFAULT 'proposed' | **New (Fix #27).** `proposed` (found by gap analysis, not yet reviewed) / `mapped` (`SOURCE_MAPPING` resolves, ready to drive report generation) / `gap` (Fix #28 — a required field with no current source in `VIGIL.CORE`; deliberate and non-blocking, not an error). A transition is a new row via `GOVERNANCE_WRITE`, same key, later `LOADED_AT` — never an `UPDATE`. |
| `SOURCE_MAPPING` | VARCHAR | Free text (e.g. `TRADES.PRICE`, `MARKET_PARTICIPANTS.REGULATORY_ATTRIBUTES:lei`) — same caveat as `OBLIGATION_MAP.SOURCE_TABLE`/`SOURCE_COLUMNS` (Fix #9): unenforceable as a real FK against a dynamic column reference. Must resolve against `INFORMATION_SCHEMA` before `STATUS` can be written as `mapped`; `NULL` by convention when `STATUS = 'gap'` (not a resolution failure — there is nothing to resolve). |
| `FIELD_FORMAT` | VARCHAR | e.g. `ISO8601`, `ISIN`, `decimal(18,4)`, or a named code-lookup. |
| `IS_REQUIRED` | BOOLEAN NOT NULL | Drives `TRANSACTION_REPORTS.FIELDS_COMPLETE` (Fix #22, #29 — only among `STATUS = 'mapped'` rows) and `REPORT_TEMPLATE_COVERAGE` (Fix #28). |
| `CREATED_AT` | TIMESTAMP_NTZ NOT NULL | |
| `CREATED_BY` | VARCHAR NOT NULL | |
| `LOADED_AT` | TIMESTAMP_NTZ NOT NULL | A circular amending the required field list, or a `gap` field getting mapped once an adaptor/schema change ships, is a new row, same (`JURISDICTION_ID`, `REPORT_TYPE`, `FIELD_NAME`), later `LOADED_AT` — never an `UPDATE`. This is exactly the scenario milestoning exists for: a historical submission must remain checkable against the template version actually in force when it was submitted. |
| `LOADED_BY` | VARCHAR NOT NULL | |

PK: (`JURISDICTION_ID`, `REPORT_TYPE`, `FIELD_NAME`, `LOADED_AT`). `REPORT_TEMPLATES_CURRENT` view = latest `LOADED_AT` per (`JURISDICTION_ID`, `REPORT_TYPE`, `FIELD_NAME`).

### `REPORT_TEMPLATE_COVERAGE` — new view, per Fix #28
One row per (`JURISDICTION_ID`, `REPORT_TYPE`): `PCT_REQUIRED_FIELDS_MAPPED` (the fraction of `REPORT_TEMPLATES_CURRENT WHERE IS_REQUIRED` rows with `STATUS = 'mapped'`) plus the list of `FIELD_NAME`s currently `STATUS = 'gap'`. Same "surfaced, not hidden" discipline as `WASH_DETECTION_COVERAGE` (Fix #3) — a required field the schema can't yet source is a visible, queryable fact, not a silent omission from `FIELDS_COMPLETE`'s denominator. `rule-interpret`/`assure-report` surface this figure alongside any completeness result, so "this report is complete" and "this report is complete among the fields we can currently capture" are never the same claim.

### `REPORT_TEMPLATE_RULE_CHUNKS` — new, per Fix #21; mirrors `OBLIGATION_RULE_CHUNKS` (Fix #9, #13)
| Column | Type | Notes |
|---|---|---|
| `JURISDICTION_ID` | VARCHAR NOT NULL | |
| `REPORT_TYPE` | VARCHAR NOT NULL | |
| `FIELD_NAME` | VARCHAR NOT NULL | References `REPORT_TEMPLATES`' natural key (not a specific `LOADED_AT` version) — same convention `OBLIGATION_RULE_CHUNKS` uses relative to `OBLIGATION_MAP`. |
| `RULE_CHUNK_ID` | VARCHAR NOT NULL | FK → `RULE_CORPUS`. |
| `IS_ACTIVE` | BOOLEAN NOT NULL DEFAULT TRUE | Tombstone pattern (Fix #13) — a wrong citation is corrected by a new row, never a `DELETE`. |
| `CREATED_AT` | TIMESTAMP_NTZ NOT NULL | |
| `CREATED_BY` | VARCHAR NOT NULL | |
| `LOADED_AT` | TIMESTAMP_NTZ NOT NULL | |
| `LOADED_BY` | VARCHAR NOT NULL | |

PK: (`JURISDICTION_ID`, `REPORT_TYPE`, `FIELD_NAME`, `RULE_CHUNK_ID`, `LOADED_AT`). `REPORT_TEMPLATE_RULE_CHUNKS_CURRENT` view = latest `LOADED_AT` per (`JURISDICTION_ID`, `REPORT_TYPE`, `FIELD_NAME`, `RULE_CHUNK_ID`) `WHERE IS_ACTIVE`. Every required field in a report template is now traceable to the specific circular paragraph that mandates it — the same citation discipline the product already applies to obligations, now applied to what a report actually has to contain.
...
- v5 — milestoning made a universal, plug-and-play property of every `VIGIL.CORE` table (`[FIX #10]`–`[FIX #19]`), prompted by a hard requirement that nothing in `VIGIL.CORE` is ever deleted or mutated. Every table now carries `CREATED_AT`/`CREATED_BY`/`LOADED_AT`/`LOADED_BY` with `LOADED_AT` in its PK and a generic `<TABLE>_CURRENT` view; no functional role is ever granted `UPDATE`/`DELETE` on anything (closing `OBLIGATION_MAP`'s one remaining `UPDATE` grant, Fix #12). `ORDERS` becomes event-sourced (`EVENT_TYPE`/`EVENT_TS` replace mutable `MODIFIED_TS`/`CANCELLED_TS`, Fix #16); `TRADES` gains a companion `TRADE_CORRECTIONS` table for busts/amendments (Fix #15) instead of ever mutating a trade print; `TRANSACTION_REPORTS` and `RULE_CORPUS` become append-only for the same reason (Fixes #14, #17); `OBLIGATION_RULE_CHUNKS` gains an `IS_ACTIVE` tombstone flag so a wrong association is corrected without `DELETE` (Fix #13); `APPROVED_OBLIGATIONS` is redefined against the latest `LOADED_AT` row per obligation (Fix #12).
- v6 — two market-structure gaps closed (`[FIX #20]`–`[FIX #26]`). `TRADES.MATCHING_MECHANISM` (Fix #20) lets the wash-trading detector exclude legitimate, disclosed internal/agency crosses from its pattern-match without excluding them from `WASH_DETECTION_COVERAGE` or downstream review. New `REPORT_TEMPLATES`/`REPORT_TEMPLATE_RULE_CHUNKS` tables (Fix #21) give the schema its first real representation of a regulator's own required report format — field-level, milestoned, and citation-backed to `RULE_CORPUS` the same way `OBLIGATION_MAP` already is — closing a gap where `TRANSACTION_REPORTS` could check timeliness and trade-match accuracy but never what the regulator's circular actually requires the report to contain. `FIELDS_COMPLETE` now has a real definition against `REPORT_TEMPLATES` (Fix #22, the same class of bug `PARAMS` closed in Fix #2). `TRANSACTION_REPORTS.TRADE_ID` becomes nullable with a new `REPORT_SCOPE` (Fix #23) so periodic/nil filings are representable; `REPORT_STATUS` (Fix #24) distinguishes an internal correction from a regulator-facing amendment/cancellation; `REPORT_PAYLOAD_REF` (Fix #25) gives the schema a place to record what was actually submitted; `REGULATORY_ATTRIBUTES VARIANT` on `ORDERS`/`TRADES` plus `DEFERRED_PUBLICATION_UNTIL` on `TRANSACTION_REPORTS` (Fix #26) cover detector-irrelevant regulatory fields (LEI, trading capacity, short-sell/algo flags) and large-in-scale block-trade disclosure deferral, both previously unrepresented.
- v7 (this version) — closes an inconsistency v6 left unresolved: what happens when a circular requires a field the schema has no source for at all (`[FIX #27]`–`[FIX #29]`). `REPORT_TEMPLATES` gains the `STATUS` (`proposed`/`mapped`/`gap`) lifecycle v6 referenced but never actually defined (Fix #27). `STATUS = 'gap'` is a deliberate, non-blocking state for a required field with no current data source — not an error, not a block, not a fabricated mapping — surfaced via a new `REPORT_TEMPLATE_COVERAGE` view (Fix #28), the same "surfaced, not hidden" discipline `WASH_DETECTION_COVERAGE` (Fix #3) already applies to wash-trading blind spots. `FIELDS_COMPLETE` (Fix #22) is narrowed to `STATUS = 'mapped'` rows only (Fix #29), so a structural gap shows up once, as a template-level finding, instead of making every report of that type permanently and uninformatively "incomplete."
